...
7. **Delete the demo world.** The vault ships with a small connected sample world (the `_EXAMPLE_*` notes in `entities/` and `gaps/` — the realm of "Valdris" shown in the screenshots above), so the dashboards, graph, and publishing layer all render with real content the moment you open it. Once you've seen how it fits together, delete every `_EXAMPLE_*` file — they're clearly marked and not part of your world. (Keep the `_TEMPLATE_*` files; those are the blanks you copy.)
